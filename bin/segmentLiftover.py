...
tmp_dir = '.tmp/'
log_dir = 'logs/'
chain_dir = '../data/liftover/chains/'
...
# 0 or -1
#
def convertProbes(fin, fo, chain, remap, remap_flag=True, force=True, new_colnames=[]):
...
#                 raise
        ...
        if not force and os.path.isfile(fo): return 0
        ...
        if (remap_flag == True) and (os.path.getsize(os.path.join(tmp_dir,'probes.unmapped')) >0):
            ...
            probes_new = pd.concat([probes_new,probes_remap])
...
@click.option('--force', 'force', type=bool, help='Determine whether to rerun if output probe files exist.')
def cli(input_dir, output_dir, chain_file, test_mode, file_indexing, segment_input_file, segment_output_file, 
        ...
        new_segment_header, new_probe_header, resume_files, liftover_path_usr, beta_usr, demo, log_path_usr, force):
    ...
    if resume_files is None: resume_files = ""
...
#            elif os.path.basename(f) == probe_input_file:
            # elif ((probe_input_file !=None) or (index_file !=None )) and (pro_pattern.match(os.path.basename(f))):
                ...
                code = convertProbes(f, probe_out_path, chain_file, remapped_list, 
                                     remap_flag, new_probe_header, force)
                if code == 0:
                    pro_succ_counter += 1
                else:
                    pro_fail_counter += 1
